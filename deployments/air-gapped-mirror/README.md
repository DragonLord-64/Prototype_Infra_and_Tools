...
for image in git-daemon devpi apt-cacher-ng sync-job; do
  docker build -t "air-gapped-mirror/$image:latest" "deployments/air-gapped-mirror/$image"
  minikube image load "air-gapped-mirror/$image:latest"
done

helm upgrade --install air-gapped-mirror ./deployments/air-gapped-mirror/chart \
...
Client machines can be configured with the central Ansible project after
replacing the example DNS and Git values in the playbook:
...
```sh
cd deployments/air-gapped-mirror/sync-job
python3 -m venv .venv
