"""Model the FHS Prometheus rules and resolve the labels they select on."""
...
import promql
from collectors import MetricLabels
...
def _device_matchers(node) -> list[str]:
    devices: list[str] = []
    for item in promql.walk(node):
        if isinstance(item, promql.Selector):
...
class RuleSet:
    ...
    def _collect_selector_sources(self) -> set[str]:
        ...
        for rule in self.rules:
            for item in promql.walk(rule.node):
                if isinstance(item, promql.Selector):
                    for matcher in item.matchers:
    ...
    def resolve_selector(self, selector: promql.Selector) -> list[str]:
    ...
    def _matches(self, rule: Rule, matcher: promql.Matcher) -> bool:
...
def _matcher_hit(matcher: promql.Matcher, value: str) -> bool:
    if matcher.op == "=":
