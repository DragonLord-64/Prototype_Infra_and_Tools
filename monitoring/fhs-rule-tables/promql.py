"""A small parser for the subset of PromQL used by the FHS rule files.

Only what the FHS threshold/aggregation rules actually use is supported:
selectors, range selectors, function calls, aggregations with by/without,
and binary operators with the ``bool``/``on``/``group_left`` modifiers.
"""
